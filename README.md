...
sb is not a git clone and does not use git's repository format. It keeps the two ideas git got right (content addressing and a Merkle DAG of snapshots) and drops the staging area, detached HEADs, destructive commands, and the loose-file repository layout.
...
Requirements: Python 3.9+ (standard library only) on Linux, macOS, or WSL. sb's symlink-safe write paths use POSIX directory descriptors, so native Windows is not supported; use WSL there.
...
sb is built on three decisions.

First, safety is structural. No sb command discards saved history. `undo` creates new history that reverts the old. `switch` refuses to run over unsaved work. The store is one SQLite database, so every operation is a single atomic transaction; a crash mid-save leaves you exactly where you were.
...
- Moving, deleting, or injecting a branch tip behind sb's back (direct SQL included) is caught by `sb verify`, which cross-checks refs against the journal.
...
With `-i` (`--ignore`), in shared mode: skip files locked by others. A merge that skipped files is recorded as partial (a single-parent save, not a merge commit), so re-running the merge after the locks release brings in the skipped files. sb does not record ancestry it didn't actually merge.
...
- Editing a file locks it to you. There is no daemon; locks are picked up the next time any sb command scans the tree, and attributed to the actual editor (below).
...
A file on disk carries no record of who edited it, so a lock found at scan time must not simply go to whoever ran the scan. If Alice edits and Bob runs `sb status`, Bob must not become the owner of Alice's edit. The signal sb uses is the file's owner uid: each teammate writes as their own OS account. sb keeps a uid → identity registry (every sb command anyone runs in shared mode records which OS account maps to which `sb who` identity) and locks each discovered edit to the account that owns the file. Someone who has never run sb still gets correct locks under their system account name, and expired-lock auto-saves are committed under the real editor's name.
...
The most common irreversible mistake in version control is committing a credential; history is permanent, and rotating a leaked key is an incident. sb scans every file being added or modified, at save time, for:
...
sb makes three promises. Each comes with its mechanism, what it defends against, and what it doesn't.
...
Does not defend against: an attacker with write access to the database and knowledge of sb's format can rewrite the entire store into a new, internally consistent history. With no secret material anywhere, internal consistency is recomputable by anyone; this is inherent to keyless designs. Anchors (Section 10) close the gap: a chain-head value recorded outside the machine cannot be reproduced by any rewrite. What sb will not do is ship the appearance of cryptographic authenticity (signatures, badges) without the key management that would make it meaningful.
...
- Access control. sb is a local tool; file permissions are the access control.
...
sb's integrity model uses no cryptographic keys (Section 9); nothing in save, merge, verify, the journal, or anchors depends on a secret. Archive confidentiality is a separate concern, handled by [vox](https://jts.gg/vox) (v1.7.3, symmetric core), a small single-file encryption module embedded inside sb and loaded into memory only while `pack`, `unpack`, or `export -k` runs. No separate file, no install step, no network. The unused legacy asymmetric interface was removed from the embedded copy in 1.2.
...
- A wrong pass-key or a single altered byte means the archive will not open. vox verifies authenticity before decrypting, and sb re-hashes the recovered payload against `db_sha256`; full-repo archives additionally go through the staged verification battery.
...
- Inspectable with standard database tooling, and anything changed with that tooling behind sb's back is flagged by `verify`.
...
Two behaviors to know, the second being a real footgun: ignored files are invisible to `save`/`status` but never deleted by sb; and ignoring an already-tracked file makes it show as `deleted`, so the next save removes it from the tree (the file stays on disk but leaves history going forward). If you meant "stop tracking but keep the file," that's what happens. If you didn't, check `status` before saving.
...
**Release with a paper trail.** Keep the real suite in `sb-tests/pre-publish/`. Ship with `sb publish v1.4`: sb verifies the store, runs the suite against a clean checkout of exactly what's shipping, and journals the record, including the content hashes of the gate scripts that ran. `sb publish -l` is the release history.
...
Summary: git is a distributed collaboration system you can also use alone; sb is a safety-and-integrity system for individuals and small teams. If you need GitHub-style multi-party collaboration today, use git, possibly with sb alongside it (they coexist; sb ignores `.git`, add `.sb` to `.gitignore`).
...
Inside test scripts, sb also exports `SB_STAGE`, `SB_BRANCH`, `SB_COMMIT`, `SB_REPO` (Section 7).
...
The split is script-friendly: `2` means sb worked correctly and blocked something on purpose, so automation can tell "fix your command" from "fix your content."
...
Copy `.sb/sandbox.db` (any time sb isn't mid-command; WAL makes even that forgiving), copy the whole project folder, or `sb pack` for an encrypted single-file backup. After restoring, run `sb verify`.
...
**Symlinks?**
Not tracked yet; skipped with a printed note. Symlinks pointing into the repo also can't hijack sb's writes: every write path refuses to follow them.
...
Alice. Locks are attributed to the file's owner on disk, resolved through the uid → identity registry, not to whoever ran sb first. Section 6 covers the edge cases.
...
**`object … does not match its hash` / `verify` reports problems** — real corruption or tampering; sb stopped rather than propagating it. Restore `.sb/sandbox.db` from a backup, then `sb verify`. Undamaged files can be rescued first via `sb undo -p` or `sb export` from saves whose objects are intact.

**`branch '…' changed under this operation … run the command again`** — two sb commands raced; yours lost the compare-and-swap and nothing was changed. Re-run it.

**`store error: … database is locked`** — another sb command has the database open for writing. Wait a moment and retry.

**`file system error: …`** — a permission or disk problem outside sb's control, reported cleanly.
